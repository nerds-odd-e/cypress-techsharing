					...
					{#each $groups as group}
						<tr class="flex flex-col flex-no wrap sm:table-row mb-2 sm:mb-0">
							...
							<td class="border-grey-light border hover:bg-gray-100 p-3 truncate"
								>{formatLocation(group)}</td
							>
							<td class="border-grey-light border hover:bg-gray-100 p-3 truncate"
								>{formatEvents(group.events)}</td
							>
							<td class="border-grey-light border hover:bg-gray-100 p-3 hover:font-medium">
								<button
									type="button"
									on:click={() => goto(`/group/${group.id}`)}
									class="bg-blue-500 text-gray-100 text-sm rounded hover:bg-blue-600 px-2 focus:outline-none cursor-pointer"
									>Edit</button
								>
								<button
									type="button"
									on:click={() => handleDelete(group.id)}
									class="bg-red-500 text-gray-100 text-sm rounded hover:bg-red-600 px-2 focus:outline-none cursor-pointer"
									>Delete</button
								>
							</td>
						</tr>
